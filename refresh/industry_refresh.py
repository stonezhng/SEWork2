...
def refresh_industry():
    ...
    for x in range(1, 29):
        ...
        updata_cmd = 'update `sw_industry` set `score` = '+str(score)+', `pb_index` = '+str(pb_index)+', `pe_index` = ' + str(pe_index) + \
    ...
    i = 1
    select_cmd = 'select `score`, `industryid` from `sw_industry`  order by `score` desc'
    cursor.execute(select_cmd)
    data = list(cursor.fetchall())
    for each in data:
        updata_cmd = 'update `sw_industry` set `rank` = '+str(i) +' where `industryid` = '+ str(each[1])
        cursor.execute(updata_cmd)
        i += 1
