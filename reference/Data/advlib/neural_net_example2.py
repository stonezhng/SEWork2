...
def get_data(id):
    ...
    startdata = '2016-05-01'
    enddate = '2016-05-20'

    select_cmd = 'select `close` from `stock_2016` where `stockid` = "'+id+'" and `date` >= "'+startdate+'" ' \
                'and `date` <= "'+enddate+'"'
...
def demo():
    ...
    n.train(pat)
    # 测试训练的成果（不要吃惊哦）
    price_per = n.test(pat)
    print price_per[0] * sum
    print close[-1]
