...
def refresh_svm(id):
    ...
    cursor = db.cursor()
    # cursor.execute('update `forecast` set `unstable` = 1 where `date`>= "2016-06-09"')
    cursor.execute('select MAX(`date`) from `forecast` where `stockid` = "'+id+'" and `unstable` = 0')
    startdate = list(cursor.fetchall())[0][0]
    if startdate is None:
        print id + ' empty table'
        return
    ...
    cursor.execute('delete from `forecast` where `stockid` = "'+id+'" and `unstable` = 1')
    cursor.execute('select `close` from `bench` where `stockid` = "' + id + '" and `date` <= "' + startdate + '" '
                ' and `date` >= "2015-01-01"')
    ...
    while start.strftime("%Y-%m-%d") != enddate:
        ...
        # print date
        cursor.execute(
            'select `close` from `bench` where `stockid` = "' + id + '" and `date` = "' + date + '"')
        tempdata = list(cursor.fetchall())
        if len(tempdata) != 0:
    ...
    # print predict
    if len(predict) == 0:
        cursor.execute('select `price_middle` from `forecast` where `stockid` = "'+id+'" order by `date` desc limit 0, 1')
        predict.append(list(cursor.fetchall())[0][0])
    for x in range(0, 15):
        date = start.strftime("%Y-%m-%d")
        temp = [id, date]
        ...
        data.append(temp)
    # print data
    insert_cmd = 'insert into `forecast` (`stockid`, `date`, `price_middle`, `price_high`, `price_low`, `unstable`) ' \
                 ' VALUES (%s, %s, %s, %s, %s, %s)'
    cursor.executemany(insert_cmd, tuple(data))
    db.commit()
    db.close()
...
refresh_svm('hs300')
refresh_svm('sh000001')
refresh_svm('sz399001')
refresh_bp('hs300')
refresh_bp('sh000001')
refresh_bp('sz399001')